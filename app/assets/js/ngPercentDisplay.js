...
//                 jEle.find('.ngpercentdisplay').css({ 'width': side, 'height': side, 'font-size': '15px', 'background-color': colors[0], 'color': colors[1] });
                ...
                $scope.$watch('percent', function(newvalue, oldvalue){
                    ...
                    if (newvalue > -1 && newvalue < 101) {
                        if(newvalue <= 50) {
                            ...
                            deg = 180 - (newvalue / 100 * 360);
                            strdeg = "rotateZ(-" + deg + "deg)";
                            rightSide.css({ "transform": strdeg, "-webkit-transform": strdeg, "-moz-transform": strdeg, "msTransform": "rotate(-" + deg + "deg)", "background-image": "linear-gradient(#41ad3f, #41ad3f)" });
                        } else {
                            // Adjust left
                            leftSide.show();
                            deg = 180 - ((newvalue - 50) / 100 * 360);
                            strdeg = "rotateZ(-" + deg + "deg)";
                            if(deg >= 90){
                                leftSide.css({ "transform": strdeg, "-webkit-transform": strdeg, "-moz-transform": strdeg, "msTransform": "rotate(-" + deg + "deg)","background-image": "linear-gradient(#ca730a, #fe9000)" });
                                rightSide.css({ "background-image": "linear-gradient(#ca730a, #ca730a)"  });
                            }
                            else{
                                leftSide.css({ "transform": strdeg, "-webkit-transform": strdeg, "-moz-transform": strdeg, "msTransform": "rotate(-" + deg + "deg)","background-image": "linear-gradient(#cf2828, #cf2828)" });
                                rightSide.css({ "background-image": "linear-gradient(#cf2828, #cf2828)"  });
                            }
                            // leftSide.css({ "transform": strdeg, "-webkit-transform": strdeg, "-moz-transform": strdeg, "msTransform": "rotate(-" + deg + "deg)","background-image": "linear-gradient(#ca730a, #fe9000)" });
                            rightSide.css({ "transform": "rotateZ(0deg)", "-webkit-transform": "rotateZ(0deg)", "-moz-transform": "rotateZ(0deg)", "msTransform": "rotate(0deg)"  });
                        }
